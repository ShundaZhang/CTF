...
def printable(s):
	...
	while i < len(s):
		...
		if chr(x0) in 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ,.[]_=-?!{}%^&*()#@':
		#if chr(x0) in 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789':
		#if chr(x0) in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
			r += chr(x0)
		else:
			return False
